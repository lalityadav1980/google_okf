# Verity Knowledge Fabric

Governed, versioned knowledge for enterprise agents.

This repository contains a technology-industry-grade proposal and reference
framework for adopting Google Cloud's Open Knowledge Format (OKF) as a
versioned, portable knowledge interchange standard across an enterprise.
...
An adopting organisation should use OKF as the **canonical exchange and
controlled release format** for agent-ready knowledge. It should not replace Confluence,
...
3. [Use-case catalog](docs/03-use-cases.md) — prioritized enterprise-wide use cases
...
6. [VerityKF Enterprise Profile](docs/06-verity-kf-profile.md) — proposed concept
...
24. [Delivery status and decisions](docs/18-delivery-status-and-decisions.md) —
    stage gates, verified evidence, organisation-owned blockers, and restart sequence.
...
26. [ADR-0007](docs/adr/0007-verity-knowledge-fabric-project-identity.md) — project
    brand, canonical identifiers, breaking migration, and external approval gate.
...
uv run verity-kf allocate-identity \
  examples/rendering/source-record.yaml \
  profiles/verity-kf-identity.yaml \
  --type Runbook
uv run verity-kf hash-concept \
  examples/rendering/expected/runbooks/identity-service-degradation--178875d5e353.md
uv run verity-kf build-release examples/pilot-bundle \
  --profile profiles/verity-kf-pilot.yaml \
  --bundle-id verity-kf-pilot \
...
- typed OKF v0.2 and VerityKF Enterprise Profile models;
...
- the VerityKF Enterprise Profile;
...
assumed to satisfy an adopting organisation's production, security, resilience,
