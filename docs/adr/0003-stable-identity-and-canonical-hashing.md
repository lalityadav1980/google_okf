...
The adopting organisation requires stable lineage across title changes, reorganizations, source
...
2. **`concept_uid`:** immutable VerityKF extension identifying the conceptual
...
`profiles/verity-kf-identity.yaml`. The UUID name is canonical JSON containing:
...
`urn:verity-kf:concept:<uuid>`. Source fragments must be durable semantic keys,
...
- **`verity-kf-concept-c14n-v1`:** parses YAML frontmatter, normalizes Unicode to
...
- **`verity-kf-source-c14n-v1`:** applies the same typed canonical encoding to all
...
- The adopting organisation owns an extension profile and must govern namespace/policy versions.
